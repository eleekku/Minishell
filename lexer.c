...
void take_d_quote(t_token *token, t_char_iter *iter, int *d_flag)
{
    token->pos.start = char_iter_cursor(iter);
    token->pos.len = 0;
    if (*d_flag == 0)
...
void    take_dollar(t_token *token, t_char_iter *iter, int *d_flag)
{
    token->type = TOKEN_DOLAR;
    token->pos.start = char_iter_cursor(iter);
    token->pos.len = 1;
...
void    take_string(t_token *token, t_char_iter *iter, int *d_flag)
{
    token->type = TOKEN_STR;
    token->pos.start = char_iter_cursor(iter);
    token->pos.len = 0;
...
void    add_token(t_token *token, t_char_iter *iter, int *d_flag)
{
    if (iter->start[0] == '<' && *d_flag % 2 == 0)
        ...
        take_d_quote(token, iter, d_flag);
        ...
        take_dollar(token, iter, d_flag);
    else if (iter->start[0] == '|' && *d_flag % 2 == 0)
        take_pipe(token, iter);
    else
        take_string(token, iter, d_flag);
}
